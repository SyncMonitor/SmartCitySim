...
public interface ParkingAreaStatsRepository extends CrudRepository<ParkingAreaStats, Long> {
	...
	@Query("select s from ParkingAreaStats s where id = ?1 order by id desc")
	public ParkingAreaStats getParkingAreaStatsById(Long id);
	
	@Query("select s from ParkingAreaStats s where fkSensorId = ?1 order by id desc")
	public List<ParkingAreaStats> getParkingAreaStatsBySensorId(Long sensorId);
	
	@Query("select s from ParkingAreaStats s where lastUpdate >= ?1 order by id desc")
	...
	@Query("select s from ParkingAreaStats s where fkSensorId = ?1 and lastUpdate > ?2 and lastUpdate < ?3")
...
//	Delete
	...
	@Modifying
	@Query("delete ParkingAreaStats s where lastUpdate < ?1")
